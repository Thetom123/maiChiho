...
(async function () {
  ...
  function alignRewardsToBlocks(processedRewards, blocks) {
    ...
    blocks.forEach((block, idx) => {
      ...
      const dist = m ? parseInt(m[1].replace(/,/g, ''), 10) : null;
      domBlocks.push({ block, originalIdx: idx, distance: dist });
    });
    ...
    processedRewards.forEach((r, idx) => {
      const match = domBlocks.find(d => d.distance === r.cumulative);
      if (match) {
        wikiToDomMap[idx] = match.originalIdx;
      }
    });

    // 容錯處理：如果有未對齊的項目，且 processedRewards 長度與 blocks 長度相同，則使用順序對齊
  ...
  `;
    ...
    function refreshUI() {
      ...
      if (isReprint) {
        // 復刻模式：只保留網頁中實際存在的獎勵區塊（因為部分歌曲與里程碑會被拔除）
        const keepIndices = [];
        processedRewards.forEach((r, idx) => {
